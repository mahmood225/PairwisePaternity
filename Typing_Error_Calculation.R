...
#----------------------------------------------------------------------------------
Typing_Error_Calculation <- function(STR,Min_genotyped_allele= 7 ,
                                     ...
                                     Error_Gradient_Calulation=FALSE){
  STR[] <- sapply(STR[],as.character)
  STR=STR[,c("ID", "Sex", "DamID","SireID",colnames(STR)[which(!colnames(STR)%in%c("ID", "Sex", "DamID","SireID", "Date"))])]
  STR_Names=colnames(STR)[which(!colnames(STR)%in%c("ID","Sex", "DamID", "SireID"))]
  if(Father_Offspring_Calulation){
    STR=STR[rowSums(!is.na(STR[,STR_Names]))>=1,]
  }else{
    STR=STR[rowSums(!is.na(STR[,STR_Names]))>=Min_genotyped_allele,]
  }
  ...
  if(Mother_Offspring_Calulation){
    ...
    STR_mother=STR_mother[rowSums(!is.na(STR_mother[,STR_Names]))>=Min_genotyped_allele,]
  ...
  if(Father_Offspring_Calulation){
    ...
    STR_father=STR_father[rowSums(!is.na(STR_father[,STR_Names]))>=Min_genotyped_allele,]
    if(!Ensure_Correctness){
  ...
  if(Error_Gradient_Calulation){
    ...
    if(Mother_Offspring_Calulation & Father_Offspring_Calulation)ind=c(unlist(as.vector(gradient_mother)),unlist(as.vector(gradient_father)))
    if(Mother_Offspring_Calulation & !Father_Offspring_Calulation)ind=unlist(as.vector(gradient_mother))
    if(!Mother_Offspring_Calulation & Father_Offspring_Calulation)ind=unlist(as.vector(gradient_father))
    ind=ind[!is.na(ind)]
    ind2=data.frame(Number=1:length(ind),Distance=ind)
    ggplot(data=ind2,aes(x=Distance) ) +geom_line(stat="density") #you can use this . 
    ind2=table(ind)
    ind2=data.frame(num_mut=ind2)
    colnames(ind2)=c("Mutation_type","Numbre")
  }
  if(Error_Gradient_Calulation) return(ind2) else return(Type_Error) 
}
